...
function resolveDashboardApiConfig() {
    ...
    const baseUrl =
        ...
        (window.location.protocol.startsWith('http') ? window.location.origin : 'http://127.0.0.1:5001');
...
function getApiCandidateUrls(path) {
    ...
    if (DASHBOARD_API_CONFIG.baseUrl) candidates.push(DASHBOARD_API_CONFIG.baseUrl + cleanPath);
    if (window.location.protocol.startsWith('http')) candidates.push(window.location.origin.replace(/\/$/, '') + cleanPath);
...
function normalizeNewsFeedItems(articles) {
    return articles.map(article => {
        const isPositive = article.sentiment.includes('positive') || article.sentiment.includes('bull');
        const isNegative = article.sentiment.includes('negative') || article.sentiment.includes('bear');
        return {
...
function startLiveFeed() {
    setInterval(() => {
        ...
        div.innerHTML = `
            <span class="feed-icon muted">DATA</span>
            <div class="feed-body">
                <div class="feed-text">Live feed unavailable: connect a news or social data provider</div>
                <div class="feed-time">live status</div>
            </div>`;
